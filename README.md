...
- 准备台词的默认提示词更新；旧配置中仍保持默认值的准备台词 AI 提示词会自动迁移到新版默认提示词；用户自定义提示词不会被覆盖。
- 对自定义牌组 README 说明进行了整理。
### 1.1.0
...
- 插件会先确定本次真实抽牌结果，再发送准备台词；用户看到的顺序仍是准备台词在前。
...
每个自定义牌组放在 `tarot_jsons/<牌组名>` 下。插件只会把包含 `tarots.json` 的目录识别为牌组；`tarot_jsons/formation.json` 是所有牌组共用的牌阵文件，不需要在每个牌组目录里重复放置。
...
最少只需要准备 `tarot_jsons/<牌组名>/tarots.json`。如果要发送牌面图片，再为每张牌准备正逆位图片：
...
这里的 `<牌名>` 默认使用 `tarots.json` 里的 `name`。插件也兼容常见标准牌别名，例如 `ACE` 会匹配 `王牌`，`2`–`10` 会匹配 `二`–`十`，`隐士` 会匹配 `隐者`，所以 `name: "圣杯2"` 可以读取 `圣杯二正位.jpg` / `圣杯二逆位.jpg`。

`tarots.json` 用于描述牌名和正逆位牌义。标准牌可以继续使用 `0`–`77` 作为键，插件会自动推断标准编号和大小阿卡纳分类：
...
自定义 ID 也可以使用，但建议显式填写 `standard_id` 和 `arcana`：
...
- `name` 必填，会作为对外显示的牌名，也会用于查找图片文件。
- `info.description` 必填，作为正位牌义。
- `info.reverseDescription` 必填，作为逆位牌义。
...
- `_meta` 可选，只用于描述牌组；如果填写 `total_cards`，插件会在数量不一致时记录 warning，不会阻止加载。
