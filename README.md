...
- 修复新版 SDK 下 AI 调用把模型任务名误当成具体模型名的问题，改用 `task_name` 传递配置，解决“未找到名为 'replyer' 的模型”报错。
- **仅在通过 Planner 调用 `tarots` 工具触发占卜时**，使用顶层 `stop_after_execution` 替换无效的 `metadata.pause_execution`；在支持该字段的 MaiBot（1.2.4 起）中，工具成功后会在当前工具批次完成时结束 Planner，减少后续追加解读。自然语言规则直接拦截和 `/塔罗` 命令入口不使用此字段，仍按原有机制处理。
- 停止字段仅作为补充保护，保留原有自然语言规则优先拦截、Planner 兜底和避免额外回复的文字约束；不会取消同批次已安排的回复，也不阻止新消息触发的后续处理。
- 准备台词模板加入 `{user_request}` 占位符来复述请求内容，以方便区分单人的多次请求结果；AI 默认提示词会用自己的话简短概括本次占卜问题。旧默认模板和提示词自动迁移，自定义内容保留。Planner 入口使用工具传入的请求内容。

#### ⚠️ 升级提醒：自定义过准备台词的用户请检查

> [!IMPORTANT]
> **如果你修改过“准备台词模板”或“准备台词提示词”，请参考下方例文手动调整。**
> 本次新增占卜问题概括，自定义设置不会被自动覆盖；仍使用已知旧默认值的用户会自动更新。

本次准备台词包含的内容有变：除了表示开始抽牌，还会复述本次占卜请求，方便在多次结果先后返回时辨认对应的问题。

**本次更新仅在内容与已知旧默认值完全一致时自动更新，包括此前“自然复述”版本的默认 AI 提示词；自定义准备台词模板或 AI 提示词始终保留，不再因配置版本号不同而强制覆盖。** 如果你设置过自定义内容，推荐参考以下例文重新改写，并保留自己喜欢的语气。

**自定义准备台词模板**（关闭“AI 生成准备台词”时使用，也用于 AI 生成失败时的回退）：

```text
好的，我来为这件事抽牌：{user_request}
```

`{user_request}` 会替换为本次占卜请求；为空时使用“本次占卜”。你也可以继续使用 `{user}`、`{card_type}`、`{formation}`。例如：

```text
这次想问的是“{user_request}”，我来用{formation}看看。
```

**AI 准备台词提示词示例**（开启“AI 生成准备台词”时使用）：

```text
{bot_style_context}

请生成一句简短的占卜前准备台词，建议20-60字。
先用自己的话简短概括用户想占卜的问题，不照抄或逐字复述请求原文，再表达开始准备、洗牌或抽牌。
...
请将示例填写到对应的“准备台词模板”或“准备台词提示词”设置项中。插件不会再在生成后统一追加固定请求标签；自定义内容需要自行包含 `{user_request}` 或相应的复述要求。通过 Planner 触发时，请求内容来自工具参数，可能是模型概括后的表述。

### 1.1.4

- 改进 MaiBot 人格风格重写阶段：准备台词会带入专用重写约束，避免风格重写时新增牌名、正逆位、牌阵位置、牌义或结果倾向。
- 保留牌义解读等其它阶段的原有事实保持规则，不关闭人格风格渲染。

### 1.1.3

- 修复 1.1.2 中已保存的准备台词 AI 提示词可能继续覆盖新版默认提示词，导致 `{cards_info}` 没有进入实际设置的问题。
- 本次升级会作为特殊情况强制覆盖 `preface_prompt`，确保准备台词能拿到本次真实抽牌事实，避免继续编造牌面。
- 如果你手动修改过准备台词 AI 提示词，请在更新前自行备份当前设置；本次覆盖完成后，后续再修改提示词不会被反复覆盖。

### 1.1.2

- 修复准备台词虽然已在真实抽牌后生成，但提示词仍未接收本次抽牌事实，导致 AI 仍可能编造牌面的问题。
- 准备台词默认提示词新增内部抽牌事实占位符，仅用于避免编造，不会要求提前公布牌名、正逆位或解读。
